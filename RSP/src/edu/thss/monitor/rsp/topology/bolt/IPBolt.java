 ...
 */
public class IPBolt extends TimeRecordBolt {
	...
	public void execute(Tuple input) {
		...
		try {
			...
			Date timestamp = new Date(parsedData.getTimestamp()); // 当前时间
			...
			if (preRecord == null) {
				ChangeInfo changeInfo = new ChangeInfo(ChangeInfo.CHANGETYPE_UPDATE, "device2ip", device.getDeviceID(),
						parsedData.getIp() + "#" + dFormat.format(timestamp));
				...
				if (!(currentIP.equals(preIP)) || (timestamp.getTime() - preTimestamp.getTime()) > 120000) {
					ChangeInfo changeInfo = new ChangeInfo(ChangeInfo.CHANGETYPE_UPDATE, "device2ip",
							device.getDeviceID(), parsedData.getIp() + "#" + dFormat.format(timestamp));
					RegionalRC.syncChange(changeInfo);
				}
			}
			Date end = new Date();
			TimeLogger.recordTime(device.getDeviceID(), ComponentId.IP_BOLT, start.getTime(), end.getTime());
